...
const cucumberJsonDir = (process.cwd(), "cypress/\cucumber-json")
const cucumberReportFileMap = {};
const cucumberReportMap = {};
const jsonIndentLevel = 2;
const htmlReportDir = (process.cwd(),"HTMLReports")
const screenshotsDir = path.resolve(process.cwd(), "cypress/\screenshots");


function getConfigurationByFile(file)
{
    const pathToConfigFile = path.resolve("cypress","config",file);
    return fs.readJson(pathToConfigFile)
}

module.exports = (on,config)=>{
  on('file:preprocessor',cucumber())
  const environment = process.env.NODE_ENV || 'production'; // default to Production if not Set
  const file = environment === 'production' ? 'production.json' : 'stage.json';

  //const file="dev-store.json";
  on('task', {
  ...
  on('before:run',(results)=>{
    rmdir(htmlReportDir, { maxRetries: 10, recursive: true }, (err) => {
        if (err) {
            console.error(err)
        }
    })
...
function generateReport() {
  if (!fs.existsSync(cucumberJsonDir)) {
    ...
    report.generate({
      ...
      reportName: `Automation Testing SUITE - ${new Date().toLocaleString()}`,
      metadata: {
        app: {
          name: "Fluree Sense Automation",
          version: "1.0.0",
        },
        browser: {
          name: "CHROME",
